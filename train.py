...
def _make_gaussian_mask(H, W, gaze_xy, sigma_frac=0.25, device="cpu"):
    """Create a soft Gaussian mask centered at gaze_xy (normalized [0,1]) with sigma as a
    fraction of the image size. Returns (1,1,H,W)."""
    gx = gaze_xy[0, 0].clamp(0, 1) * (W - 1)
    gy = gaze_xy[0, 1].clamp(0, 1) * (H - 1)
    yy, xx = torch.meshgrid(torch.arange(H, device=device), torch.arange(W, device=device), indexing="ij")
    dx2 = (xx - gx) ** 2
    dy2 = (yy - gy) ** 2
    sigma = sigma_frac * min(H, W)
    sigma2 = max(1e-6, sigma * sigma)
    mask = torch.exp(-(dx2 + dy2) / (2.0 * sigma2))
...
def train(use_pretrained_decoder=True, load_full_model=False, no_rl=False):
    ...
    policy_params = list(model.policy_head.parameters()) + list(model.value_head.parameters())
    backbone_params = [p for n, p in model.named_parameters() if not (n.startswith('policy_head') or n.startswith('value_head'))]
    ...
    detach_until = int(getattr(Config, 'RL_DETACH_UNTIL_EPISODE', 0))
    ramp_eps = int(getattr(Config, 'RL_RAMP_EPISODES', 0))
    ...
    for epoch in range(Config.EPOCHS):
        ...
        for batch_idx, (images, _) in enumerate(train_loader):
            ...
            gaze = torch.rand(1, 2, device=Config.DEVICE) * 0.6 + 0.2  # Start in central 60% area

            # Initialize LSTM memory
            state = model.init_memory(1, Config.DEVICE)
            reconstruction = None

            if episodes_seen < detach_until:
                attach_alpha = 0.0
            else:
                if ramp_eps > 0:
                    attach_alpha = min(1.0, max(0.0, (episodes_seen - detach_until) / float(ramp_eps)))
                else:
                    attach_alpha = 1.0

            # Log current coupling (only relevant when RL enabled)
            if not no_rl:
                try:
                    writer.add_scalar("RL/attach_alpha", float(attach_alpha), global_step)
                except Exception:
                    pass
            ...
            for step in range(num_steps):
                ...
                patch = crop_patch(image, gaze, Config.FOVEA_CROP_SIZE, resize_to=Config.FOVEA_OUTPUT_SIZE)
                reconstruction, state = model(patch, state, gaze)
                ...
                if getattr(Config, "USE_MASKED_STEP_LOSS", False):
                    ...
                    mask = _make_gaussian_mask(H, W, gaze, sigma_frac=sigma_frac, device=Config.DEVICE)
                    # Broadcast to channels
                    mask3 = mask.expand(1, 3, H, W)
                    mse_m = ((reconstruction - image) ** 2 * mask3).sum() / (mask3.sum() + 1e-6)
                    l1_m = (reconstruction - image).abs().mul(mask3).sum() / (mask3.sum() + 1e-6)
                else:
                    mse_m = 0
                    l1_m = 0
                ...
                # ----- RL: compute reward from improvement in FULL reconstruction loss -----
                with torch.no_grad():
                    # full (unmasked) step loss
                    full_mse = mse_loss(reconstruction, image)
                    full_l1 = l1_loss(reconstruction, image)
                    curr_full_err = Config.MSE_WEIGHT * full_mse + Config.L1_WEIGHT * full_l1
                    if prev_full_err is None:
                        # first step: just set baseline, don't add reward (since no rl action was taken yet)
                        prev_full_err = curr_full_err
                    else:
                        r_t = (prev_full_err - curr_full_err)
                        rewards.append(r_t)
                        prev_full_err = curr_full_err
                ...
                if not no_rl:
                    ...
                    h_t = state[0][-1]  # (B,H)
                    # Blend detached and attached hidden state for smooth transition
                    h_for_policy = h_t.detach() * (1.0 - attach_alpha) + h_t * attach_alpha
                    logits, v_t = model.policy_value(h_for_policy, gaze)  # logits (B,8), value (B,)
            ...
            if (not no_rl) and len(rewards) > 0:
                ...
                rewards_t_raw = torch.stack(rewards)
                scale = float(getattr(Config, 'RL_REWARD_SCALE', 1.0))
                rewards_t = rewards_t_raw * scale  # (T, B=1)
                logprobs_t = torch.stack(logprobs)  # (T, B=1)
                values_t = torch.stack(values)      # (T, B=1)
                entropies_t = torch.stack(entropies)  # (T, B=1)

                with torch.no_grad():
                    # bootstrap with critic using final hidden state and current gaze
                    h_T = state[0][-1]
                    # Even though we're in no_grad, keep the same coupling for consistency
                    h_T_for_policy = h_T.detach() * (1.0 - attach_alpha) + h_T * attach_alpha
                    next_value = model.policy_value(h_T_for_policy, gaze)[1]  # (B,)
                    T = rewards_t.size(0)
            ...
            total_loss.backward()
            # Gradient clipping for stability with deeper LSTM/decoder
            ...
            if batch_idx % 10 == 0:
                grid = vutils.make_grid(torch.cat([image, reconstruction.detach()], dim=0), nrow=2, normalize=True)
                writer.add_image("Episode/original_vs_reconstruction", grid, global_step)
                # Also log gaze positions overlayed on the original image
                try:
                    H, W = Config.IMG_SIZE
                    ...
                    if len(pts) > 0:
                        xs, ys = zip(*pts)
                        ax.scatter(xs, ys, c='r', s=40, marker='x')
                        # connect points to show path
                        ax.plot(xs, ys, c='yellow', linewidth=1, alpha=0.8)
                    ax.set_title('Gaze positions (episode)')
                    ax.axis('off')
                    writer.add_figure("Episode/gaze_positions", fig, global_step)
                    plt.close(fig)
                except Exception:
                    pass
                print(f"Epoch {epoch} Batch {batch_idx}: rec_loss={total_rec_loss.item():.6f} total={total_loss.item():.6f}")
