...
def seed_snake(image, mask,
    ...
    while trial_count <= max_segment_trial:
        ...
        if (stop_with_availability is not None) and \
...
def draw_line_n_mask(im_size, start_coord, orientation, length, thickness, margin, large_dilation_struct, aa_scale, contrast_scale=1.0):
    ...
    # Replace scipy.misc.imresize with PIL resize and fix np.float
    miniline_im_pil = miniline_blown_im.resize((miniline_shape, miniline_shape), Image.LANCZOS)
    miniline_im = np.array(miniline_im_pil).astype(float)/255
    ...
    # Replace scipy.misc.imresize with PIL resize and fix np.float
    minimask_im_pil = Image.fromarray(minimask_blown_im).resize((miniline_shape, miniline_shape), Image.LANCZOS)
    minimask_im = np.array(minimask_im_pil).astype(float) / 255
...
def test():
    ...
    show1_pil = Image.fromarray((red_target * 255).astype(np.uint8)).resize((imsize, imsize), Image.LANCZOS)
    show1 = np.array(show1_pil) / 255.0
    plt.imshow(show1)
    plt.axis('off')
    ...
    # Replace scipy.misc.imresize with PIL resize
    show2_pil = Image.fromarray((gray_total * 255).astype(np.uint8)).resize((imsize, imsize), Image.LANCZOS)
    show2 = np.array(show2_pil) / 255.0
    plt.imshow(show2)
    plt.axis('off')
...
def from_wrapper(args):
    ...
    while (iimg < args.n_images):
        ...
        if (args.save_images):
            ...
            # Replace scipy.misc.imsave with PIL save
            final_im_pil = Image.fromarray((final_im * 255).astype(np.uint8))
            final_im_pil.save(os.path.join(args.contour_path, contour_sub_path, fn))
        if (args.save_gt):
            fn = "gt_%s.png"%(iimg)
            # Replace scipy.misc.imsave with PIL save
            target_im_pil = Image.fromarray((target_im * 255).astype(np.uint8))
            target_im_pil.save(os.path.join(args.contour_path, gt_sub_path, fn))
        if (args.save_metadata):
