...
def draw_circle(window_size, coordinate, radius, aa_scale):
    ...
    image_pil = Image.fromarray((image * 255).astype(np.uint8)).resize(window_size, Image.LANCZOS)
    return np.array(image_pil).astype(float) 
...
def from_wrapper(args):
    ...
    while (iimg < args.n_images):
        ...
        if args.segmentation_task:
            marker = origin_circle.astype(float) / 255
            merker2 = terminal_circle.astype(float) / 255
        ...
        '''
        Debugging added by me, not needed anymore
        
        
        print(f"Origin coord: {origin_mark_coord}")
        print(f"Terminal coord: {terminal_mark_coord}")
        print(f"Marker radius: {args.marker_radius}")
        print(f"Origin circle max: {np.max(origin_circle)}")
        print(f"Terminal circle max: {np.max(terminal_circle)}")'''

        if (args.pause_display):
            plt.figure(figsize=(10, 10))
            # Replace scipy.misc.imresize with PIL resize
            show2_pil = Image.fromarray((image_marked * 255).astype(np.uint8)).resize((args.window_size[0], args.window_size[1]), Image.LANCZOS)
        ...
        if args.segmentation_task:
            if (args.save_images):
                ...
                image_marked_pil = Image.fromarray((image_marked * 255).astype(np.uint8))
                image_marked_pil.save(os.path.join(args.contour_path, contour_sub_path, fn))
                target_segment_pil = Image.fromarray((target_segment * 255).astype(np.uint8))
                target_segment_pil.save(os.path.join(args.contour_path, seg_sub_path, fn))
            if (args.save_metadata):
